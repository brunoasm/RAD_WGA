...
for infile in ../../all_weevils/demultiplexed/*
do
export infile
export sample=`basename $infile | cut -d _ -f 1`
echo processing ${sample}, file $infile 
sbatch -N 1 -n 8 -p serial_requeue -o $sample.%A.out --export=ALL --mem=5000 -J r_$sample -t 1-00:00:00 count.sh
done
