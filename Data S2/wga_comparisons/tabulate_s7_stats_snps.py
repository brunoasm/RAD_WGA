...
#written by B. Medeiros Apr-2017
...
##function to parse a *.loci file and record snps only
...
def loci_file_parser(filename):
    ...
    def separator_parser(line): #this parses separator lines. Returns number of locus and position of SNPS
        line = re.sub('[>\n]','',line)
        line_split = re.split('\|',line)
        locus_number = line_split[1]
        snps_pos = [i-18 for i in xrange(len(line_split[0])) if (i >= 18 and line_split[0][i] != ' ')]
        return({'locus_number':locus_number, 'snps_pos':snps_pos})
    with open(filename, 'r') as infile:
        ...
        for line in infile:
            if '//' not in line: #while parsing a single RAD locus, create a temporary dictionary
                ...
                locus_info = separator_parser(line)
                locus_id = locus_info['locus_number']
                if locus_info['snps_pos']:
                    temp_dict = {(sampleid, pool, locus_id):''.join([sequence[i] for i in locus_info['snps_pos']]) for [(sampleid, pool),sequence] in temp_dict.iteritems()}
...
#calculates, the number of sites with same genotype call, including gaps and ambiguities
def N_same(sample_pool_1, sample_pool_2, seq_dict):
    #samples and pools have to be provided as lists of tuples. E.g. ('BdM1000', '10')
    nuc_matching = []
    nuc_all = []
    loci = tuple(set(locus_key for (samp_key, pool_key, locus_key) in seq_dict.iterkeys())) #record loci names
    for locus in loci:
        ...
        if seq1_test and seq2_test:
            for i in xrange(len(seq_dict[(sample_pool_1[0], sample_pool_1[1], locus)])):
                ...
                nucs = (seq_dict[(sample_pool_1[0], sample_pool_1[1], locus)][i],
                        seq_dict[(sample_pool_2[0], sample_pool_2[1], locus)][i])
                if nucs[0] == nucs[1]: #if alleles are the same, count 1
                    temp_matching.append(1)
                else: #if not the same, count 0
                    temp_matching.append(0)
        if len(temp_matching):
            nuc_matching.append(float(sum(temp_matching)))
            nuc_all.append(len(temp_matching)) 
        ...
        for j in sample_info.index:
            if sample_info.loc[i,'taxon'] == sample_info.loc[j,'taxon']: #only compute if same taxon
                ...
                try: #will fail if some of the samples not in loci file
                    ...
                    pairwise_comp[(sample_pool_1, sample_pool_2, 'N_same')] = N_same(sample_pool_1, sample_pool_2, taxon_dict)
...
if __name__ == "__main__":    
    ...
    sample_info.loc[:,'N_snps_s7'] = np.nan
    sample_info.loc[:,'N_snps_heteroz'] = np.nan
    sample_info.loc[:,'N_snps_ambiguous'] = np.nan
    sample_info.loc[:,'snps_GC_content'] = np.nan
    ...
    for stats in results:
        ...
        sample_info.loc[stats[0],'N_snps_s7'] = stats[2]
        sample_info.loc[stats[0],'N_snps_heteroz'] = stats[3]
        sample_info.loc[stats[0],'N_snps_ambiguous'] = stats[4]
        sample_info.loc[stats[0],'snps_GC_content'] = stats[5]
    ...
    sample_info.to_csv(os.path.basename(args.info).split('.')[0] + '_snps_WGA.csv', index = False)
    ...
    for stat in stats_to_record:
        ...
        temp_dataframe.to_csv('pairwise_snps_{}.csv'.format(stat))
